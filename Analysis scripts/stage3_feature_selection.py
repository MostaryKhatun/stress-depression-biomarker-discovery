...
# List all Fisher score files to display options to the user
datasets = [f for f in os.listdir(score_dir) if f.endswith('_Fisher_Scores.csv') and not f.startswith('._')]
# datasets = [f for f in os.listdir(score_dir)]
dataset_names = [os.path.splitext(f)[0].replace('_Fisher_Scores', '') for f in datasets]
...
for idx in selected_indices:
    ...
    sorted_feature_scores = feature_scores.sort_values(by='Fisher Score', ascending=False)
    ...
    if choice == 1:
        ...
        selected_features = sorted_feature_scores[sorted_feature_scores['Fisher Score'] >= threshold]['Feature']
